...
class StringSet3 {
	...
	public StringSet3 union(StringSet3 set) {
		...
		newSet.list.addAll(this.list); for (String s : set.list) if (newSet.contains(s) == false) newSet.add(s);
		return newSet;
	}

	public StringSet3 intersection(StringSet3 set) {
		StringSet3 newSet = new StringSet3(); for (String s : list) if (set.contains(s)) newSet.add(s);
		return newSet;
	}

	public StringSet3 difference(StringSet3 set) {
		StringSet3 newSet = new StringSet3(); for (String s : list) if (set.contains(s) == false) newSet.add(s);
		return newSet;
	}
